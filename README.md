...
> Для примера, пробую [на учебном проекте в отдельной ветке](https://github.com/comerc/round-squares/tree/2FA).
...
Вы читаете эти файлы. Не нравится библиотека для OTP? Пишете в чате: _"Замени библиотеку X на Y в дизайне"_. AI правит md-файлы. Код еще не написан, а ошибки уже исправлены! Прелесть в том, что не надо как-то дополнительно уточнять, что мы редактируем спецификацию. AI продолжает работать в режиме планирования. Например: я внёс уточнения, что хочу "email-based OTP" без хранения email и с конфигом для `mailpit`.
...
**5. Реализация (Code):**
Когда спек утвержден, вы даете команду: _"Реализуй всё это!"_. (Шорткат для профи: `/openspec:apply add-2fa`). AI берет `tasks.md` и идет по пунктам, отмечая галочками сделанное.

**6. Проверка (Code-Review):**
В идеале все таски должны быть закрыты, но мы же знаем, как это бывает. Командуете пройти по `tasks.md` и проставить галочки, что сделано. Я обнаружил, что остаются хвосты. А время уже вышло.
